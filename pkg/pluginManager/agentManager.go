...
func (s *pluginManager) RunAgentController() {
	...
	if !slices.Contains(types.TaskKinds, types.AgentConfig.TaskKind) && !types.AgentConfig.GeneralAgent {
	...
	for name, plugin := range s.chainingPlugins {
		if name != types.AgentConfig.TaskKind && !types.AgentConfig.GeneralAgent {
			continue
		}
